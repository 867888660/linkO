...
def get_llm_api_key():
    """
    从环境变量读取当前组件的 API 密钥
    根据 Edit/Edit.json 中的 llmMappings 配置查找对应的密钥名称
    """
    # 1. 获取当前组件名称（去除 .py 后缀）
    component_name = Path(__file__).stem  # DeepSeek.py -> DeepSeek
    
    # 2. 读取 Edit/Edit.json 获取映射关系
    try:
        # 假设 Edit.json 在项目根目录下
            ...
            with open(edit_path, 'r', encoding='utf-8') as f:
                ...
                if key_name:
                    ...
                    if api_key:
                        ...
                        print(f"⚠️ 警告: 环境变量 '{key_name}' 未设置，组件: {component_name}")
    except Exception as e:
        print(f"⚠️ 读取 Edit.json 失败: {e}")
    
    # 4. Fallback: 如果找不到，返回空字符串（或可以返回默认值）
    return ""
...
def run_node(node):
    ...
    # 从环境变量读取密钥（通过 Edit/Edit.json 配置）
