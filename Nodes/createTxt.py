...
# Helper to resolve alias paths like @TempFiles to absolute paths under project root
def _resolve_alias_path(path_str: str) -> str:
    if not isinstance(path_str, str):
        return path_str
    try:
        base_dir = Path(__file__).resolve().parents[1]  # project root (folder that contains Nodes)
        alias_map = {
            '@TempFiles': base_dir / '@TempFiles',
            '@NoteBook':  base_dir / 'NoteBook',
            '@Memory':    base_dir / 'Memory',
            '@WorkFlow':  base_dir / 'WorkFlow',
            '@Nodes':     base_dir / 'Nodes',
        }
        for alias, real_base in alias_map.items():
            if path_str.startswith(alias):
                sub = path_str[len(alias):].lstrip('\\/')
                return str((real_base / sub).resolve())
    except Exception:
        pass
    return path_str
...
def run_node(node):
    try:
        ...
        # Resolve alias path (e.g., @TempFiles) to absolute path under project root
        file_path = _resolve_alias_path(file_path)
